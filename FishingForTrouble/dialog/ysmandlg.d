...
//Men
IF ~RandomNum(14,0)~ THEN BEGIN men1
  	SAY @2020
    IF~~THEN EXIT
END
...
IF ~Global("ystalk","GLOBAL",1)~ THEN BEGIN men51
    SAY @2028
    IF~~THEN
...
IF ~Global("ystalk","GLOBAL",1)~ THEN BEGIN men52
    SAY @2029
    IF~~THEN
      DO ~SetGlobal("ystalk","GLOBAL",0)~
    EXIT
...
IF ~Global("ystalk","GLOBAL",1)~ THEN BEGIN men61
    SAY @2034
    IF~~THEN
      DO ~SetGlobal("ystalk","GLOBAL",0)~
    EXIT
...
IF ~Global("ystalk","GLOBAL",1)~ THEN BEGIN men62
    SAY @2035
    IF~~THEN
      DO ~SetGlobal("ystalk","GLOBAL",0)~
    EXIT
